# USPTO Provisional Patent Filing Guide
## SuvrenHOA — Blockchain-Based HOA Governance System

**Inventor:** Ryan Shanahan, Raleigh, NC  
**Applicant:** Suvren LLC (or Ryan Shanahan if LLC not yet formed)  
**Date Prepared:** April 2026

---

## Pre-Filing Checklist

### Create Your USPTO Account
- Go to: **https://patentcenter.uspto.gov**
- Click "Register" and create a USPTO.gov account
- Verify your email before proceeding

### Verify Micro Entity Status
You qualify as a **micro entity** if ALL of the following are true:
- [ ] You haven't been named as an inventor on more than **4 prior U.S. patent applications**
- [ ] Your gross income in the prior calendar year was **under ~$230,000** (adjusted periodically by USPTO — verify current threshold at filing time)
- [ ] You have **not assigned, granted, conveyed, or licensed** (and are not obligated to assign) rights in the invention to an entity with gross income over the threshold
- [ ] The entity claiming micro entity status is not a large entity

✅ Filing as micro entity saves **50%** on fees: **$160 instead of $320**.

### Documents to Have Ready
| File | Description |
|------|-------------|
| `SuvrenHOA-Provisional-Patent.pdf` | The 24-claim patent specification |
| `SuvrenHOA-Patent-Drawings.pdf` | All 7 USPTO-compliant technical drawings |
| `SuvrenHOA-Code-Appendix.pdf` | Smart contract source code appendix |

---

## Filing Steps

### Step 1 — Log Into Patent Center
- URL: **https://patentcenter.uspto.gov**
- Sign in with your USPTO.gov credentials

### Step 2 — Start New Submission
1. Click **"New Submission"**
2. Select **"Patent"**
3. Select **"Provisional Application"**
4. Click **"Start"**

### Step 3 — Enter Inventor Information
- **First Name:** Ryan  
- **Last Name:** Shanahan  
- **City:** Raleigh  
- **State:** NC  
- **Country:** US  
- **Citizenship:** US

> Add additional inventors if applicable. For SuvrenHOA, Ryan Shanahan is the sole inventor.

### Step 4 — Enter Applicant Information
**If Suvren LLC is formed:**
- Applicant Type: **Assignee (Legal Entity)**
- Entity Name: **Suvren LLC**
- City/State: Raleigh, NC
- Entity Size: **Micro Entity** (see certification below)

**If Suvren LLC is NOT yet formed:**
- Applicant Type: **Inventor**
- Name: **Ryan Shanahan**
- You can assign to Suvren LLC after it's formed — the provisional establishes priority, not ownership

### Step 5 — Upload Specification
- Click **"Add Document"** → **"Specification"**
- Upload: `SuvrenHOA-Provisional-Patent.pdf`
- Document description: "Specification"

> The specification is your core patent document — the 24-claim draft describing the blockchain-based HOA governance system.

### Step 6 — Upload Drawings
- Click **"Add Document"** → **"Drawings"**
- Upload: `SuvrenHOA-Patent-Drawings.pdf`
- Document description: "Drawings"

> All 7 figures must be in the drawings file:
> - FIG. 1 — System Architecture Overview
> - FIG. 2 — PropertyNFT Lifecycle State Diagram
> - FIG. 3 — Governance Proposal State Machine
> - FIG. 4 — Treasury Fund Flow Diagram
> - FIG. 5 — Document Registration and Verification Workflow
> - FIG. 6 — Community Health Score Computation Diagram
> - FIG. 7 — Frontend Application Architecture

### Step 7 — Upload Code Appendix
- Click **"Add Document"** → **"Exhibit/Appendix"**
- Upload: `SuvrenHOA-Code-Appendix.pdf`
- Document description: "Appendix A — Smart Contract Source Code"

### Step 8 — File Micro Entity Certification
- In the fee section, select **"Micro Entity"**
- Check the certification box confirming you meet micro entity requirements (see checklist above)
- This must be filed **before or at the time of payment** to receive the discount

> ⚠️ False micro entity certification is fraud. If unsure, file as **small entity** (~$320) or consult an attorney.

### Step 9 — Pay Filing Fee
- Fee: **$160** (micro entity provisional application, 2025 fee schedule — verify current amount)
- Payment: Credit card accepted in Patent Center
- Review order summary before submitting

### Step 10 — Submit and Download Receipt
1. Review all uploaded documents one final time
2. Click **"Submit"**
3. **IMMEDIATELY download the filing receipt PDF**
4. The receipt contains:
   - Your **Application Number** (e.g., 63/XXX,XXX)
   - Your **Priority Date** (the date that matters for "who invented it first")
   - Confirmation of fee payment

---

## After Filing

### Immediately
- [ ] Save the filing receipt PDF in at least 2 places (local + cloud)
- [ ] Note the application number: `63/___________`
- [ ] Note the filing date (priority date): `____________`

### Within 1 Month
- [ ] Set a **12-month calendar reminder** — this is your non-provisional deadline
  - Example: if filed April 10, 2026 → non-provisional due by **April 10, 2027**
  - Missing this date forfeits the priority date permanently

### Within 6–8 Months
- [ ] Start engaging a **patent attorney** for non-provisional conversion
  - Budget: **$5,000–$15,000** for a software/blockchain patent attorney
  - Look for attorneys with blockchain or fintech IP experience
  - They'll need your provisional, drawings, and code appendix
  - Earlier is better — give them time to do a proper job

### At 12 Months — Non-Provisional Deadline
- [ ] File non-provisional patent application **before** the 12-month anniversary
- [ ] Non-provisional claims must be supported by the provisional specification
- [ ] Fee: ~$800–$1,600 depending on entity size and claim count

---

## Important Notes

### Priority Date
The provisional filing establishes your **priority date** — the legally recognized date of your invention for purposes of patent examination. If anyone else files a similar patent after your priority date, your filing wins. **This is why you file now, even with a rough specification.**

### What a Provisional Does NOT Do
- It does NOT get examined by a patent examiner
- It does NOT become a patent automatically
- It does NOT publish (remains confidential)
- It DOES establish your priority date for 12 months

### Soulbound Transfers Are Novel
The key patentable novelty in SuvrenHOA includes:
1. Soulbound ERC-721 tokens representing real property ownership with governance-approved transfer gates
2. On-chain HOA governance with tiered quorum (15%/33%/51%/67%) per proposal category
3. Community micro-loan system backed by HOA reserve fund
4. Automated USDC yield generation (Aave integration) governed by risk tolerance levels
5. Immutable document registry with cryptographic hash verification

### Entity Formation
If Suvren LLC is not yet formed at time of filing, file as individual inventor (Ryan Shanahan). After LLC formation, file an **Assignment** document with the USPTO transferring the provisional application to Suvren LLC. This is standard practice and preserves the priority date.

### Confidentiality After Filing
Once filed, do **not** publicly disclose the full patent specification before the non-provisional publishes (typically 18 months after non-provisional filing). NDAs should cover any third parties you show the technology to in the interim.

---

## Fee Reference (verify current amounts at USPTO.gov/PatentFees)

| Fee Type | Micro Entity | Small Entity | Large Entity |
|----------|-------------|--------------|--------------|
| Provisional Application | ~$160 | ~$320 | ~$640 |
| Non-Provisional Basic | ~$420 | ~$840 | ~$1,680 |
| Search Fee | ~$285 | ~$570 | ~$1,140 |
| Examination Fee | ~$185 | ~$370 | ~$740 |

> Fees are set by USPTO and adjusted periodically. Always verify at **https://www.uspto.gov/learning-and-resources/fees-and-payment/uspto-fee-schedule** before filing.

---

## Contact & Resources

- **USPTO Patent Center:** https://patentcenter.uspto.gov
- **USPTO Fee Schedule:** https://www.uspto.gov/learning-and-resources/fees-and-payment/uspto-fee-schedule
- **Micro Entity Certification (37 CFR 1.29):** https://www.ecfr.gov/current/title-37/chapter-I/subchapter-A/part-1/subpart-A/section-1.29
- **Provisional Application Guide:** https://www.uspto.gov/patents/basics/types-patent-applications/provisional-application-patent

---

*This guide is for informational purposes and does not constitute legal advice. Consult a registered patent attorney or agent for legal counsel on your specific situation.*
