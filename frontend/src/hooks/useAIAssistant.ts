    ...
    async (text: string): Promise<string> => {
      ...
      if (/my dues|how much do i owe|dues status|am i current|paid up/.test(q)) {
        ...
        if (dues.isCurrent === false) {
          return ` You have **${dues.quartersOwed} quarter${dues.quartersOwed !== 1 ? 's' : ''}** of dues outstanding — **$${dues.amountOwed} USDC** total. Head to [Pay Dues](/dues) to get current.`;
        }
      ...
      if (/proposal|active.*vote|vote.*active|governance|pending.*proposal/.test(q)) {
        ...
        if (count === 0) {
          return ` There are currently **no active proposals**. Check [/proposals](/proposals) to see past proposals or create a new one.`;
        }
        return ` There are **${count} active proposal${count !== 1 ? 's' : ''}** open for voting right now! Head to [/proposals](/proposals) to review and cast your vote.`;
      }
