'use client';

import { useActivityFeed, type ActivityItem } from '@/hooks/useActivityFeed';
import { useMemo } from 'react';

const BASESCAN = 'https://sepolia.basescan.org/tx';

function timeAgo(unix: number): string {
  const diff = Math.floor(Date.now() / 1000) - unix;
  if (diff < 60) return `${Math.max(0, diff)}s ago`;
  if (diff < 3600) return `${Math.floor(diff / 60)}m ago`;
  if (diff < 86400) return `${Math.floor(diff / 3600)}h ago`;
  return `${Math.floor(diff / 86400)}d ago`;
}

function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      <div className="w-12 h-12 rounded-xl bg-[rgba(176,155,113,0.10)] border border-[rgba(176,155,113,0.20)] flex items-center justify-center text-2xl mb-4">
        
      </div>
      <p className="text-sm text-[var(--text-disabled)] font-medium">No recent activity</p>
      <p className="text-xs text-[var(--text-disabled)] mt-1">Events will appear here as they happen</p>
    </div>
  );
}

function SkeletonRow() {
  return (
    <div className="flex items-center gap-3 px-4 py-3 animate-pulse">
      <div className="w-8 h-8 rounded-lg bg-[rgba(245,240,232,0.05)] shrink-0" />
      <div className="flex-1 min-w-0">
        <div className="h-3 bg-[rgba(245,240,232,0.05)] rounded-full w-3/4 mb-2" />
        <div className="h-2 bg-[rgba(245,240,232,0.05)] rounded-full w-1/3" />
      </div>
      <div className="h-2 bg-[rgba(245,240,232,0.05)] rounded-full w-12 shrink-0" />
    </div>
  );
}

interface EventRowProps {
  item: ActivityItem;
  isNew: boolean;
}

function EventRow({ item, isNew }: EventRowProps) {
  const handleClick = () => {
    window.open(`${BASESCAN}/${item.txHash}`, '_blank', 'noopener,noreferrer');
  };

  return (
    <button
      onClick={handleClick}
      className={[
        'w-full text-left flex items-center gap-3 px-4 py-3 rounded-xl',
        'transition-all duration-300 group cursor-pointer',
        'hover:bg-[rgba(245,240,232,0.05)] hover:border-[rgba(176,155,113,0.20)]',
        'border border-transparent',
        isNew
          ? 'bg-[rgba(176,155,113,0.10)] border-[rgba(176,155,113,0.25)] shadow-[0_0_12px_rgba(201,169,110,0.12)] activity-ticker-slide-in'
          : 'activity-ticker-slide-in',
      ].join(' ')}
      style={{ animationFillMode: 'both' }}
      aria-label={`${item.description} — view on BaseScan`}
    >
      {/* Icon */}
      <div className={[
        'w-8 h-8 rounded-lg flex items-center justify-center text-base shrink-0',
        'transition-transform duration-200 group-hover:scale-110',
        isNew ? 'bg-[rgba(176,155,113,0.12)] border border-[rgba(176,155,113,0.30)]' : 'bg-[rgba(245,240,232,0.05)] border border-[rgba(245,240,232,0.10)]',
      ].join(' ')}>
        {item.icon}
      </div>

      {/* Description */}
      <div className="flex-1 min-w-0">
        <p className={[
          'text-xs font-medium leading-snug truncate',
          isNew ? 'text-[#D4C4A0]' : 'text-[var(--text-body)] group-hover:text-[var(--parchment)]',
          'transition-colors duration-200',
        ].join(' ')}>
          {item.description}
        </p>
        <p className="text-[10px] text-[var(--text-disabled)] mt-0.5 truncate">
          Tx: {item.txHash.slice(0, 10)}...{item.txHash.slice(-6)}
        </p>
      </div>

      {/* Timestamp */}
      <div className="shrink-0 flex flex-col items-end gap-1">
        <span className="text-[10px] text-[var(--text-disabled)] whitespace-nowrap">
          {timeAgo(item.timestamp)}
        </span>
        {isNew && (
          <span className="text-[9px] font-medium text-[#B09B71] uppercase tracking-wider">New</span>
        )}
      </div>
    </button>
  );
}

export interface ActivityTickerProps {
  className?: string;
  maxHeight?: string;
}

export function ActivityTicker({ className = '', maxHeight = '480px' }: ActivityTickerProps) {
  const { events, isLoading, lastFetched, newIds } = useActivityFeed();

  return (
    <div className={`glass-card rounded-xl overflow-hidden ${className}`}>
      {/* Header */}
      <div className="flex items-center justify-between px-5 py-4 border-b border-[rgba(245,240,232,0.08)]">
        <div className="flex items-center gap-2.5">
          {/* Pulsing live dot — verdigris */}
          <div className="relative flex items-center justify-center">
            <span className="absolute inline-flex w-3 h-3 rounded-full animate-ping" style={{ background: 'rgba(42,93,79,0.30)' }} />
            <span className="relative inline-flex rounded-full" style={{ width: '6px', height: '6px', background: '#2A5D4F' }} />
          </div>
          <h3 className="text-sm font-medium text-[var(--parchment)] tracking-wide">Live Activity</h3>
        </div>

        <div className="flex items-center gap-2">
          {lastFetched && (
            <span className="text-[10px] text-[var(--text-disabled)] hidden sm:block">
              Updated {timeAgo(Math.floor(lastFetched.getTime() / 1000))}
            </span>
          )}
          {events.length > 0 && (
            <span className="text-[10px] text-[var(--text-disabled)] bg-[rgba(245,240,232,0.05)] px-2 py-0.5 rounded-full border border-[rgba(245,240,232,0.08)]">
              {events.length} events
            </span>
          )}
        </div>
      </div>

      {/* Content */}
      <div
        className="overflow-y-auto divide-y divide-[rgba(245,240,232,0.05)] scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent"
        style={{ maxHeight }}
      >
        {isLoading ? (
          <div className="divide-y divide-[rgba(245,240,232,0.05)]">
            {Array.from({ length: 6 }).map((_, i) => (
              <SkeletonRow key={i} />
            ))}
          </div>
        ) : events.length === 0 ? (
          <EmptyState />
        ) : (
          events.map((item) => (
            <EventRow key={item.id} item={item} isNew={newIds.has(item.id)} />
          ))
        )}
      </div>

      {/* Footer */}
      {!isLoading && events.length > 0 && (
        <div className="px-5 py-3 border-t border-[rgba(245,240,232,0.08)] flex items-center justify-between">
          <span className="text-[10px] text-[var(--text-disabled)]">
            Showing last {events.length} events · Auto-refreshes every 30s
          </span>
          <a
            href="https://sepolia.basescan.org"
            target="_blank"
            rel="noopener noreferrer"
            className="text-[10px] text-[rgba(176,155,113,0.70)] hover:text-[#B09B71] transition-colors duration-150"
          >
            View all on BaseScan →
          </a>
        </div>
      )}
    </div>
  );
}

export default ActivityTicker;
