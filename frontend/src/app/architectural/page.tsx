  ...
  if (!isConnected) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        <p className="text-[rgba(245,240,232,0.50)] mb-4">Sign in to submit architectural requests</p>
...
const PIPELINE_STAGES = [
  { key: 'submitted', label: 'Submitted', icon: '', color: 'text-[#B09B71]', bg: 'bg-yellow-500/10 border-yellow-500/20' },
  { key: 'under-review', label: 'Under Review', icon: '', color: 'text-[#5A7A9A]', bg: 'bg-[rgba(90,122,154,0.10)] border-[rgba(90,122,154,0.20)]' },
...
function ReviewPipeline({ requests, setFilter }: { requests: any[]; setFilter: (f: string) => void }) {
  ...
  return (
    <div className="glass-card rounded-xl p-4 mb-6">
      <p className="text-xs text-[rgba(245,240,232,0.35)] font-medium uppercase tracking-wider mb-3">Review Pipeline</p>
...
function RequestList({ filter, setFilter }: { filter: string; setFilter: (f: string) => void }) {
  ...
  return (
    <>
        ...
        {statuses.map(s => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-medium whitespace-nowrap transition-all ${
              filter === s ? 'bg-[#B09B71]/15 text-[#B09B71] border border-[#B09B71]/30' : 'glass-card text-[rgba(245,240,232,0.50)]'
            }`}
          >
      ...
      {isLoading ? (
        <div className="text-center py-12 text-[rgba(245,240,232,0.35)]">Loading requests...</div>
      ) : !requests || requests.length === 0 ? (
        <div className="glass-card rounded-xl p-12 text-center">
          <Building2 className="w-8 h-8 text-[rgba(245,240,232,0.50)] mx-auto mb-4" />
...
function RequestCard({ request }: { request: any }) {
  ...
  const colorClass = status.color === 'green' ? 'text-[#3A7D6F] bg-[rgba(42,93,79,0.10)] border-[rgba(42,93,79,0.20)]' :
    ...
    status.color === 'blue' ? 'text-[#5A7A9A] bg-[rgba(90,122,154,0.10)] border-[rgba(90,122,154,0.20)]' :
    status.color === 'yellow' ? 'text-[#B09B71] bg-yellow-500/10 border-yellow-500/20' :
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1.5">
              <span className="text-[10px] font-mono text-[rgba(245,240,232,0.35)]">{request.request_number}</span>
              <span className={`text-[10px] px-2 py-0.5 rounded-full border font-medium ${colorClass}`}>
                {status.label}
              </span>
              <span className="text-[10px] text-[rgba(245,240,232,0.35)]">{modType?.icon} {modType?.label}</span>
            </div>
          ...
          <div className="mt-4 pt-4 border-t border-white/5 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              {request.contractor_name && (
                <div><span className="text-[rgba(245,240,232,0.35)]">Contractor:</span> <span className="text-[rgba(245,240,232,0.65)]">{request.contractor_name}</span></div>
              )}
              {request.start_date && (
                <div><span className="text-[rgba(245,240,232,0.35)]">Start:</span> <span className="text-[rgba(245,240,232,0.65)]">{request.start_date}</span></div>
              )}
              {request.completion_date && (
                <div><span className="text-[rgba(245,240,232,0.35)]">Completion:</span> <span className="text-[rgba(245,240,232,0.65)]">{request.completion_date}</span></div>
              )}
            </div>
            ...
            {request.conditions && (
              <div className="p-3 rounded-lg bg-[#B09B71]/5 border border-amber-500/10">
                ...
                <p className="text-xs text-[rgba(245,240,232,0.50)]">{request.conditions}</p>
              </div>
            )}

            {request.reviewer_notes && (
              <div className="p-3 rounded-lg bg-gray-800/30">
                <p className="text-xs text-[rgba(245,240,232,0.35)] font-medium mb-1">Board Notes:</p>
                <p className="text-xs text-[rgba(245,240,232,0.50)]">{request.reviewer_notes}</p>
              </div>
            )}

            {comments.length > 0 && (
              <div className="space-y-2">
                <p className="text-[10px] uppercase tracking-wider text-[rgba(245,240,232,0.35)] font-semibold">Comments</p>
                {comments.map((c: any) => (
                  <div key={c.id} className="pl-3 border-l-2 border-[#B09B71]/20">
                    <p className="text-xs text-[rgba(245,240,232,0.50)]">{c.text}</p>
                    <p className="text-[10px] text-[rgba(245,240,232,0.25)] mt-0.5">{new Date(c.created_at).toLocaleDateString()}</p>
                  </div>
                ))}
              </div>
            )}
...
function SubmitForm({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="glass-card rounded-xl p-6 space-y-5">
      ...
      <div>
        <label className="block text-sm text-[rgba(245,240,232,0.50)] mb-2">Modification Type</label>
          ...
          {MOD_TYPES.map(m => (
            <button key={m.id} onClick={() => setModType(m.id)}
              className={`p-2.5 rounded-xl text-[11px] font-medium transition-all flex flex-col items-center gap-1 ${
                modType === m.id ? 'bg-[#B09B71]/15 text-[#B09B71] border border-[#B09B71]/30' : 'glass-card text-[rgba(245,240,232,0.50)]'
              }`}>
      ...
      <div>
        <label className="block text-sm text-[rgba(245,240,232,0.50)] mb-2">What are you planning?</label>
      ...
      <div>
        <label className="block text-sm text-[rgba(245,240,232,0.50)] mb-2">Details</label>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div>
          <label className="block text-sm text-[rgba(245,240,232,0.50)] mb-2">Estimated Cost</label>
          <input type="text" value={estimatedCost} onChange={e => setEstimatedCost(e.target.value)}
            placeholder="$5,000"
            className="w-full px-4 py-3 rounded-xl bg-gray-800/80 border border-gray-700 text-sm focus:border-[#B09B71]/50 focus:outline-none" />
        </div>
        <div>
          <label className="block text-sm text-[rgba(245,240,232,0.50)] mb-2">Contractor</label>
          <input type="text" value={contractorName} onChange={e => setContractorName(e.target.value)}
            placeholder="Company name"
            className="w-full px-4 py-3 rounded-xl bg-gray-800/80 border border-gray-700 text-sm focus:border-[#B09B71]/50 focus:outline-none" />
        </div>
        <div>
          <label className="block text-sm text-[rgba(245,240,232,0.50)] mb-2">Target Start</label>
