      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs">
        {segments.map((seg) => (
          <div key={seg.name} className="flex items-center gap-2">
            ...
            <span className="text-[rgba(245,240,232,0.50)]">{seg.name}</span>
            <span className="text-[rgba(245,240,232,0.35)] font-mono">{(seg.pct * 100).toFixed(0)}%</span>
          </div>
        ))}
      </div>
    </div>
...
export default function BudgetPlannerPage() {
  ...
  if (!isConnected) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        <p className="text-[rgba(245,240,232,0.50)] mb-4">Sign in to view the budget planner</p>
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10 page-enter">
      {/* Header */}
      <div className="mb-8">
        <Link href="/treasury" className="flex items-center gap-1 text-xs text-[rgba(245,240,232,0.35)] hover:text-[#B09B71] mb-4 transition-colors">
          <ChevronLeft className="w-3 h-3" /> Back to Treasury
        </Link>
        <p className="text-xs tracking-widest uppercase text-[rgba(245,240,232,0.35)] mb-1">Annual Planning</p>
        <h1 className="text-3xl font-normal tracking-tight">Budget Planner</h1>
        <p className="text-[rgba(245,240,232,0.50)] text-sm mt-2">Plan and allocate annual HOA budget — FY 2026</p>
      </div>

      {/* Summary */}
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="glass-card rounded-2xl p-5">
          <p className="text-xs text-[rgba(245,240,232,0.35)] uppercase tracking-wide mb-2">FY 2026 Total</p>
          <p className="text-2xl font-normal text-[#B09B71]">${total.toLocaleString()}</p>
        </div>
        <div className="glass-card rounded-2xl p-5">
          <p className="text-xs text-[rgba(245,240,232,0.35)] uppercase tracking-wide mb-2">FY 2025 Total</p>
          <p className="text-2xl font-normal text-[rgba(245,240,232,0.50)]">${lastTotal.toLocaleString()}</p>
        </div>
        <div className="glass-card rounded-2xl p-5">
          <p className="text-xs text-[rgba(245,240,232,0.35)] uppercase tracking-wide mb-2">YoY Change</p>
          <p className={`text-2xl font-normal ${diff > 0 ? 'text-[#8B5A5A]' : diff < 0 ? 'text-[#3A7D6F]' : 'text-[rgba(245,240,232,0.50)]'}`}>
      ...
      <div className="glass-card rounded-2xl p-7 mb-8">
        <h2 className="text-sm font-bold text-[#D4C4A0] mb-5">Budget Allocation</h2>
      ...
      <div className="glass-card rounded-2xl overflow-hidden mb-6">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[oklch(0.18_0.005_60)]">
          <h2 className="text-sm font-bold text-[#D4C4A0]">Category Breakdown</h2>
          <div className="flex gap-2">
            {editing ? (
              <>
                <button
                  onClick={() => { setBudget(loadBudget()); setEditing(false); }}
                  className="text-xs px-3 py-1.5 rounded-lg text-[rgba(245,240,232,0.35)] hover:text-[rgba(245,240,232,0.65)] border border-gray-700/50 transition-colors"
                >
                  Cancel
                </button>
                <button
              ...
              <div key={cat.name} className="flex items-center gap-4 px-6 py-4 hover:bg-white/[0.02] transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-medium text-[rgba(245,240,232,0.80)]">{cat.name}</span>
                    <div className="flex items-center gap-4">
                      {editing ? (
                        <input
                          ...
                          className="w-28 px-2 py-1 text-right text-sm bg-white/5 border border-[#B09B71]/30 rounded-lg text-[#D4C4A0] focus:outline-none focus:border-[#B09B71]/60"
                        />
                      ) : (
                        <span className="text-sm font-bold text-[#D4C4A0]">${cat.thisYear.toLocaleString()}</span>
                      )}
                      <span className={`text-xs font-medium w-16 text-right ${yoyDiff > 0 ? 'text-[#8B5A5A]' : yoyDiff < 0 ? 'text-[#3A7D6F]' : 'text-[rgba(245,240,232,0.25)]'}`}>
                        {yoyDiff >= 0 ? '+' : ''}${yoyDiff.toLocaleString()}
                      </span>
                      <span className="text-xs text-[rgba(245,240,232,0.35)] w-10 text-right">{pct.toFixed(0)}%</span>
                    </div>
                  </div>
        ...
        <div className="px-6 py-4 bg-white/[0.02] border-t border-[oklch(0.18_0.005_60)] flex items-center justify-between">
          <span className="text-sm font-bold text-[rgba(245,240,232,0.65)]">Total Budget</span>
          <span className="text-lg font-normal text-[#B09B71]">${total.toLocaleString()}</span>
        </div>
      </div>

      {saved && (
        <div className="fixed bottom-6 right-6 bg-[#B09B71] text-[#1a1a1a] px-4 py-2.5 rounded-xl text-sm font-bold shadow-lg">
           Budget saved!
        </div>
      )}
    </div>
