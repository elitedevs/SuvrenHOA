...
export default function LostFoundPage() {
  ...
  if (!isConnected) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh]">
      <p className="text-[rgba(245,240,232,0.50)] mb-4">Sign in to view Lost & Found</p>
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8 page-enter">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-[rgba(245,240,232,0.50)] mt-1">Post and find lost items in the community</p>
        </div>
      ...
      {showForm && (
        <div className="glass-card rounded-xl p-6 space-y-4 mb-6">
              ...
              <button key={t} onClick={() => setForm({...form, type: t})}
                className={`flex-1 py-2.5 rounded-xl text-sm font-medium capitalize border transition-all ${
                  form.type === t
                    ...
                    : 'glass-card text-[rgba(245,240,232,0.50)]'
                }`}>
          ...
          <div>
            <label className="block text-xs text-[rgba(245,240,232,0.50)] mb-1">Item Title</label>
            <input value={form.title} onChange={e => setForm({...form, title: e.target.value})}
              placeholder="e.g. Orange tabby cat, set of keys..." className="w-full px-3 py-2.5 rounded-xl bg-gray-800/80 border border-gray-700 text-sm focus:border-[#B09B71]/50 focus:outline-none" />
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div>
              ...
              <input type="date" value={form.date} onChange={e => setForm({...form, date: e.target.value})}
                className="w-full px-3 py-2 rounded-xl bg-gray-800/80 border border-gray-700 text-xs focus:border-[#B09B71]/50 focus:outline-none" />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-xs text-[rgba(245,240,232,0.50)] mb-1">Location</label>
              <input value={form.location} onChange={e => setForm({...form, location: e.target.value})}
                placeholder="Where it was lost/found" className="w-full px-3 py-2 rounded-xl bg-gray-800/80 border border-gray-700 text-xs focus:border-[#B09B71]/50 focus:outline-none" />
            </div>
            <div>
              ...
              <input value={form.contactLot} onChange={e => setForm({...form, contactLot: e.target.value})}
                placeholder="42" className="w-full px-3 py-2 rounded-xl bg-gray-800/80 border border-gray-700 text-xs focus:border-[#B09B71]/50 focus:outline-none" />
            </div>
          </div>
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {CATEGORIES.slice(1).map(c => (
                <button key={c} onClick={() => setForm({...form, category: c})}
                  className={`px-3 py-1.5 rounded-lg text-xs transition-all ${form.category === c ? 'bg-[#B09B71]/15 text-[#B09B71] border border-[#B09B71]/30' : 'glass-card text-[rgba(245,240,232,0.50)]'}`}>
                  {c}
                </button>
              ))}
            </div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        ...
        {CATEGORIES.map(c => (
          <button key={c} onClick={() => setCatFilter(c)}
            className={`px-3 py-1.5 rounded-lg text-xs transition-all ${catFilter === c ? 'bg-[#B09B71]/15 text-[#B09B71] border border-[#B09B71]/30' : 'glass-card text-[rgba(245,240,232,0.50)]'}`}>
            {c}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.length === 0 ? (
          <div className="col-span-full glass-card rounded-xl p-12 text-center">
            ...
            <p className="text-[rgba(245,240,232,0.50)]">No items in this category</p>
          </div>
        ) : filtered.map(item => (
            ...
            <div className="p-4">
              <div className="flex items-center gap-2 mb-2">
                <span className={`text-[10px] px-2 py-0.5 rounded-full border font-medium ${
                  item.status === 'claimed' ? 'bg-gray-500/10 text-[rgba(245,240,232,0.50)] border-gray-500/20' :
                ...
                <span className="text-[10px] text-[rgba(245,240,232,0.35)]">{item.category}</span>
              </div>
              <h3 className="font-semibold text-sm mb-1">{item.title}</h3>
              <p className="text-xs text-[rgba(245,240,232,0.50)] mb-3 leading-relaxed">{item.description}</p>
              <div className="space-y-1 text-[11px] text-[rgba(245,240,232,0.35)]">
                <p> {item.location}</p>
