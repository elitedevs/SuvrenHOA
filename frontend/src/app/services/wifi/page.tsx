  ...
  if (!isConnected) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        <p className="text-[rgba(245,240,232,0.50)] mb-4">Sign in to manage guest WiFi</p>
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8 page-enter">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-[rgba(245,240,232,0.50)] mt-1">Manage temporary WiFi access codes for community guests</p>
        </div>
        <button
      ...
      <div className="glass-card rounded-xl p-4 border border-[#B09B71]/20 mb-6 flex items-center gap-4">
        ...
        <div>
          <p className="text-xs text-[rgba(245,240,232,0.35)] uppercase tracking-wider mb-0.5">Community Guest Network</p>
          <p className="text-lg font-bold text-white">{SSID}</p>
          <p className="text-xs text-[rgba(245,240,232,0.50)]">WPA2 Protected · Guests need a code to connect</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          ...
          { label: 'Expired Codes', value: expiredCodes.length, icon: AlertTriangle, color: 'text-[rgba(245,240,232,0.35)]' },
      ...
      {/* Active Codes */}
      <h2 className="text-sm font-semibold text-[rgba(245,240,232,0.50)] uppercase tracking-wider mb-3">Active Codes</h2>
      <div className="space-y-3 mb-6">
        {activeCodes.length === 0 && (
          <div className="glass-card rounded-xl p-6 text-center">
            <Wifi className="w-8 h-8 text-[rgba(245,240,232,0.25)] mx-auto mb-2" />
            <p className="text-sm text-[rgba(245,240,232,0.50)]">No active codes — generate one for guests</p>
          </div>
        )}
          ...
          <div key={code.id} className="glass-card rounded-xl p-4 border border-green-400/10">
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1">
                ...
                <p className="text-xs text-[rgba(245,240,232,0.35)] mt-0.5">Created by {code.createdBy}</p>
                <div className="flex items-center gap-3 mt-2">
                  <span className="font-mono text-base font-bold text-[#B09B71] tracking-widest">{code.code}</span>
                  <button
                    onClick={() => copyCode(code.code)}
                    className="p-1 rounded text-[rgba(245,240,232,0.35)] hover:text-[#B09B71] transition-colors"
                    title="Copy code"
                  >
              ...
              <div className="flex flex-col items-end gap-2">
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="p-1.5 rounded-lg text-[rgba(245,240,232,0.35)] hover:text-[#B09B71] hover:bg-[#B09B71]/10 transition-colors text-xs font-medium"
                  >
                    QR
                  </button>
                  <button
                    onClick={() => deleteCode(code.id)}
                    className="p-1.5 rounded-lg text-[rgba(245,240,232,0.35)] hover:text-[#8B5A5A] hover:bg-[#8B5A5A]/10 transition-colors"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
                </div>
                <div className="flex items-center gap-1 text-xs text-[rgba(245,240,232,0.50)]">
                  <Clock className="w-3 h-3" />
                  {timeRemaining(code.expiresAt)}
                </div>
                <div className="text-xs text-[rgba(245,240,232,0.35)]">{code.uses}/{code.maxUses} uses</div>
              </div>
            </div>
            <div className="mt-2">
            ...
            {qrCode?.id === code.id && (
              <div className="mt-4 pt-4 border-t border-white/5 flex flex-col items-center gap-3">
                <p className="text-xs text-[rgba(245,240,232,0.50)]">Scan to connect to {SSID}</p>
                <div className="p-3 bg-white rounded-xl">
                  <QRCode value={wifiQRString(SSID, `${WIFI_PASSWORD_PREFIX}${code.code}`)} size={120} />
                </div>
                <p className="text-xs text-[rgba(245,240,232,0.35)]">Password: <span className="font-mono text-[rgba(245,240,232,0.65)]">{WIFI_PASSWORD_PREFIX}{code.code}</span></p>
              </div>
            )}
          </div>
        ))}
      ...
      {expiredCodes.length > 0 && (
        <>
          <h2 className="text-sm font-semibold text-[rgba(245,240,232,0.35)] uppercase tracking-wider mb-3">Expired</h2>
          <div className="space-y-2">
            {expiredCodes.map((code) => (
              <div key={code.id} className="glass-card rounded-xl p-3 border border-white/5 opacity-60">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-[rgba(245,240,232,0.50)]">{code.label}</p>
                    <p className="font-mono text-xs text-[rgba(245,240,232,0.25)] mt-0.5">{code.code}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-[#8B5A5A]">Expired</span>
                    <button onClick={() => deleteCode(code.id)} className="p-1 rounded text-[rgba(245,240,232,0.25)] hover:text-[#8B5A5A] transition-colors">
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
          <div className="glass-card rounded-2xl p-6 w-full max-w-md border border-[#B09B71]/20">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs text-[rgba(245,240,232,0.50)] mb-1 block">Label / Purpose</label>
                <input
                  value={newLabel}
              ...
              <div>
                <label className="text-xs text-[rgba(245,240,232,0.50)] mb-1 block">Expires After</label>
                <div className="flex gap-2">
                  {[4, 8, 24, 48, 168].map((h) => (
                    <button
                      key={h}
                      onClick={() => setNewDuration(h)}
                      className={`flex-1 py-2 rounded-lg text-xs font-medium transition-colors ${newDuration === h ? 'bg-[#B09B71] text-[#1a1a1a]' : 'glass-card text-[rgba(245,240,232,0.50)] hover:text-white'}`}
                    >
                      {h < 24 ? `${h}h` : `${h / 24}d`}
                    </button>
                  ))}
              ...
              <div>
                <label className="text-xs text-[rgba(245,240,232,0.50)] mb-1 block">Max Device Connections: {newMaxUses}</label>
                <input
                  type="range"
                  min={1}
                  max={50}
            ...
            <div className="flex gap-3 mt-5">
              ...
              <button onClick={createCode} className="flex-1 px-4 py-2 rounded-lg bg-[#B09B71] hover:bg-[#D4C4A0] text-[#1a1a1a] text-sm font-semibold transition-colors flex items-center justify-center gap-2">
