...
export default function ParkingPage() {
  ...
  if (!isConnected) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh]">
      <p className="text-[rgba(245,240,232,0.50)] mb-4">Sign in to manage parking</p>
...
function ParkingContent() {
  ...
  if (!loaded) return <div className="text-center py-12 text-[rgba(245,240,232,0.35)]">Loading...</div>;
  ...
  const spotColor = (status: string) => {
    if (status === 'available') return 'bg-[rgba(42,93,79,0.15)] border-[rgba(42,93,79,0.30)] text-[#3A7D6F] hover:bg-[#3A7D6F]/30';
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8 page-enter">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-[rgba(245,240,232,0.50)] mt-1">50 community parking spots — click to manage</p>
        </div>
        <div className="flex gap-2">
          {(['map', 'passes'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-all capitalize ${tab === t ? 'bg-[#B09B71]/15 text-[#B09B71] border border-[#B09B71]/30' : 'glass-card text-[rgba(245,240,232,0.50)]'}`}>
        ...
        {[
          ...
          <div key={label} className="glass-card rounded-xl p-4 text-center">
            <div className="flex items-center justify-center gap-2 mb-1">
              ...
              <span className="text-xs text-[rgba(245,240,232,0.50)]">{label}</span>
            </div>
          ...
          {selected && (
            <div className="glass-card rounded-xl p-5 space-y-4">
              ...
              {selected.status === 'available' && (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs text-[rgba(245,240,232,0.50)] mb-1">Lot Number</label>
                      <input value={assignForm.lotNumber} onChange={e => setAssignForm({ ...assignForm, lotNumber: e.target.value })}
                        placeholder="e.g. 42" className="w-full px-3 py-2 rounded-lg bg-gray-800/80 border border-gray-700 text-sm focus:border-[#B09B71]/50 focus:outline-none" />
                    </div>
                    <div>
                      <label className="block text-xs text-[rgba(245,240,232,0.50)] mb-1">Resident Name</label>
                      <input value={assignForm.name} onChange={e => setAssignForm({ ...assignForm, name: e.target.value })}
                        placeholder="Full name" className="w-full px-3 py-2 rounded-lg bg-gray-800/80 border border-gray-700 text-sm focus:border-[#B09B71]/50 focus:outline-none" />
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => {
                    ...
                    }} className="flex-1 py-2.5 rounded-xl bg-[#B09B71] hover:bg-[#D4C4A0] text-[#1a1a1a] text-sm font-medium transition-all">
                      Assign Spot
                    </button>
                    <button onClick={() => {
              ...
              {selected.status === 'assigned' && (
                <div>
                  <p className="text-sm text-[rgba(245,240,232,0.65)] mb-3">
              ...
              {selected.status === 'visitor' && selected.visitorPass && (
                <div>
                  <div className="p-3 rounded-lg bg-[#8B5A5A]/5 border border-red-500/10 mb-3">
                    <p className="text-sm text-[rgba(245,240,232,0.65)]">Visitor: <span className="text-[#8B5A5A] font-medium">{selected.visitorPass.visitorName}</span></p>
                    {selected.visitorPass.licensePlate && <p className="text-xs text-[rgba(245,240,232,0.35)]">Plate: {selected.visitorPass.licensePlate}</p>}
                    <p className="text-xs text-[rgba(245,240,232,0.35)]">Expires: {new Date(selected.visitorPass.expiresAt).toLocaleString()}</p>
                    <p className="text-xs text-[rgba(245,240,232,0.35)]">Pass ID: {selected.visitorPass.id}</p>
                  </div>
        ...
        <div className="space-y-3">
          {activePasses.length === 0 ? (
            <div className="glass-card rounded-xl p-12 text-center">
              <TicketCheck className="w-8 h-8 text-[rgba(245,240,232,0.50)] mx-auto mb-3" />
              <p className="text-[rgba(245,240,232,0.50)]">No active visitor passes</p>
            </div>
            ...
            return (
              <div key={pass.id} className="glass-card rounded-xl p-4 flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-[rgba(245,240,232,0.35)]">Spot #{pass.spotId} · Hosted by Lot #{pass.issuedBy}</p>
                  {pass.licensePlate && <p className="text-xs text-[rgba(245,240,232,0.35)]">Plate: {pass.licensePlate}</p>}
                  <p className="text-xs text-[rgba(245,240,232,0.35)]">Expires in {hoursLeft.toFixed(1)}h · {pass.id}</p>
                </div>
