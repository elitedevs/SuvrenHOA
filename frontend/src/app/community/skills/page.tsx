...
export default function SkillsExchangePage() {
  ...
  const ListingCard = ({ listing }: { listing: SkillListing }) => {
    ...
    return (
      <div className={`glass-card rounded-xl p-4 border ${listing.type === 'offer' ? 'border-green-400/15' : 'border-blue-400/15'}`}>
          ...
          <div className="glass-card rounded-2xl p-6 w-full max-w-md border border-[#B09B71]/20">
            ...
            <div className="space-y-3">
              {findMatches(selectedMatch, listings).map((m) => {
                ...
                const Icon = cfg.icon;
                return (
                  <div key={m.id} className="p-3 rounded-xl bg-[#1a1a1a] border border-white/10">
                    <div className="flex items-start gap-2.5">
