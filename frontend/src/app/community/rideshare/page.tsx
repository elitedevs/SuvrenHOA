    ...
    <div className="min-h-screen bg-[oklch(0.06_0.005_60)] p-6 lg:p-10">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8 flex items-start justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-3xl font-bold text-[#e8d5a3] mb-2 flex items-center gap-3">
              <Car className="w-8 h-8 text-[#c9a96e]" /> Community Rideshare
            </h1>
        ...
        <div className="grid grid-cols-3 gap-4 mb-7">
          {[
            { label: 'Available Rides', val: rides.length, color: 'text-[#c9a96e]' },
            { label: 'Open Seats', val: rides.reduce((a, r) => a + r.seatsLeft, 0), color: 'text-green-400' },
              ...
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-2">
                    <h3 className="font-semibold text-[#e8d5a3] text-lg">{ride.destination}</h3>
                    {ride.recurring !== 'none' && (
                      <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-[#b8942e]/20 text-[#c9a96e]">
                ...
                <div className="flex flex-col gap-2 shrink-0">
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg border border-[oklch(0.22_0.005_60)] text-[oklch(0.55_0.01_60)] hover:text-[#e8d5a3] text-sm transition-colors"
                  >
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-[oklch(0.12_0.005_60)] border border-[oklch(0.22_0.005_60)] rounded-2xl p-6 max-w-md w-full">
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-semibold text-[#e8d5a3]">Post a Ride</h3>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={form.driverLot} onChange={e => setForm({ ...form, driverLot: e.target.value })} placeholder="12" className="w-full bg-[oklch(0.16_0.005_60)] border border-[oklch(0.24_0.005_60)] rounded-lg px-3 py-2 text-sm text-[#e8d5a3] focus:outline-none focus:border-[#c9a96e]" />
              </div>
              <div>
                <label className="block text-xs text-[oklch(0.45_0.01_60)] mb-1">Destination *</label>
                <input value={form.destination} onChange={e => setForm({ ...form, destination: e.target.value })} placeholder="Publix on Congress Ave" className="w-full bg-[oklch(0.16_0.005_60)] border border-[oklch(0.24_0.005_60)] rounded-lg px-3 py-2 text-sm text-[#e8d5a3] focus:outline-none focus:border-[#c9a96e]" />
              </div>
              <div>
                <label className="block text-xs text-[oklch(0.45_0.01_60)] mb-1">Date & Time *</label>
                <input type="datetime-local" value={form.datetime} onChange={e => setForm({ ...form, datetime: e.target.value })} className="w-full bg-[oklch(0.16_0.005_60)] border border-[oklch(0.24_0.005_60)] rounded-lg px-3 py-2 text-sm text-[#e8d5a3] focus:outline-none focus:border-[#c9a96e]" />
              </div>
              <div>
                <label className="block text-xs text-[oklch(0.45_0.01_60)] mb-1">Available Seats</label>
                <input type="number" min={1} max={7} value={form.seats} onChange={e => setForm({ ...form, seats: Number(e.target.value) })} className="w-full bg-[oklch(0.16_0.005_60)] border border-[oklch(0.24_0.005_60)] rounded-lg px-3 py-2 text-sm text-[#e8d5a3] focus:outline-none focus:border-[#c9a96e]" />
              </div>
              <div>
                <label className="block text-xs text-[oklch(0.45_0.01_60)] mb-1">Recurring</label>
                <select value={form.recurring} onChange={e => setForm({ ...form, recurring: e.target.value as Ride['recurring'] })} className="w-full bg-[oklch(0.16_0.005_60)] border border-[oklch(0.24_0.005_60)] rounded-lg px-3 py-2 text-sm text-[#e8d5a3] focus:outline-none focus:border-[#c9a96e]">
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} rows={2} placeholder="Details for riders..." className="w-full bg-[oklch(0.16_0.005_60)] border border-[oklch(0.24_0.005_60)] rounded-lg px-3 py-2 text-sm text-[#e8d5a3] focus:outline-none focus:border-[#c9a96e] resize-none" />
              </div>
              <button onClick={addRide} disabled={!form.destination || !form.datetime || !form.driverLot} className="w-full py-2.5 rounded-xl bg-[#b8942e] text-[#1a1a1a] font-semibold disabled:opacity-40 hover:bg-[#c9a96e] transition-colors">
                Post Ride
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-[oklch(0.12_0.005_60)] border border-[oklch(0.22_0.005_60)] rounded-2xl p-6 max-w-sm w-full">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-[#e8d5a3]">Message {msgRide.driver}</h3>
            ...
            <textarea value={msgText} onChange={e => setMsgText(e.target.value)} rows={4} placeholder="Hi! Is there still a spot available?" className="w-full bg-[oklch(0.16_0.005_60)] border border-[oklch(0.24_0.005_60)] rounded-lg px-3 py-2 text-sm text-[#e8d5a3] focus:outline-none focus:border-[#c9a96e] resize-none mb-3" />
            <button onClick={() => { alert('Message sent! (Demo mode)'); setMsgRide(null); setMsgText(''); }} className="w-full py-2.5 rounded-xl bg-[#b8942e] text-[#1a1a1a] font-semibold hover:bg-[#c9a96e] transition-colors">
              Send Message
            </button>
          </div>
        </div>
