...
function RecipeCard({ recipe, onClick }: { recipe: Recipe; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      className="glass-card rounded-2xl p-5 border border-white/[0.04] cursor-pointer hover:border-[#c9a96e]/20 hover-lift transition-all"
    >
      <div className="text-3xl mb-3">{recipe.emoji}</div>
      <h3 className="text-base font-bold text-gray-100 mb-1">{recipe.title}</h3>
      <p className="text-xs text-gray-500 mb-3 line-clamp-2">{recipe.description}</p>
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[11px] text-gray-500 bg-white/[0.04] px-2 py-0.5 rounded-full">{recipe.cuisine}</span>
      ...
      <div className="mt-3 text-[10px] text-gray-600">Shared by {recipe.submittedBy}</div>
    </div>
  );
}
      ...
      <div className="glass w-full max-w-lg rounded-2xl border border-white/[0.08] max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-5 border-b border-white/[0.06]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-gray-100">{recipe.title}</h2>
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <span className="text-[11px] text-gray-500">{recipe.cuisine}</span>
                <span className="text-[11px] text-gray-500">{formatPrepTime(recipe.prepTime)}</span>
              </div>
            </div>
          </div>
        ...
        <div className="p-5 space-y-5">
          <p className="text-sm text-gray-400 italic">{recipe.description}</p>
          <div>
            <h3 className="text-xs font-bold text-[#c9a96e] uppercase tracking-widest mb-3">Ingredients</h3>
          ...
          <div>
            <h3 className="text-xs font-bold text-[#c9a96e] uppercase tracking-widest mb-3">Instructions</h3>
            <ol className="space-y-3">
              {recipe.steps.map((step, i) => (
                <li key={i} className="flex items-start gap-3 text-sm text-gray-300">
                  <span className="w-6 h-6 rounded-full bg-[#c9a96e]/20 text-[#c9a96e] text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
                  {step}
                </li>
              ))}
            </ol>
...
function AddRecipeModal({ onAdd, onClose }: { onAdd: (r: Recipe) => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
      <div className="glass w-full max-w-lg rounded-2xl border border-white/[0.08] max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-5 border-b border-white/[0.06]">
          <h2 className="text-base font-bold text-gray-100">Share a Recipe</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-gray-400 cursor-pointer"><X className="w-4 h-4" /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.title} onChange={e => setForm({...form, title: e.target.value})} placeholder="My Famous Chili"
                className="w-full px-3 py-2 rounded-xl bg-white/[0.04] border border-white/[0.08] text-sm text-gray-200 focus:outline-none focus:border-[#c9a96e]/50" required />
            </div>
            <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="text-xs font-semibold text-gray-400 mb-1 block">Cuisine</label>
            ...
            <div>
              <label className="text-xs font-semibold text-gray-400 mb-1 block">Prep (min)</label>
          ...
          <div>
            <label className="text-xs font-semibold text-gray-400 mb-1 block">Ingredients (one per line)</label>
            <textarea rows={4} value={form.ingredients} onChange={e => setForm({...form, ingredients: e.target.value})} placeholder="2 cups flour&#10;1 tsp salt"
              className="w-full px-3 py-2 rounded-xl bg-white/[0.04] border border-white/[0.08] text-sm text-gray-200 focus:outline-none focus:border-[#c9a96e]/50 resize-none" />
          </div>
          <div>
            <label className="text-xs font-semibold text-gray-400 mb-1 block">Steps (one per line)</label>
          ...
          <div className="flex gap-3">
            <button type="button" onClick={onClose}
              className="flex-1 py-2.5 rounded-xl border border-white/[0.08] text-sm text-gray-400 hover:text-gray-200 transition-all cursor-pointer">Cancel</button>
            <button type="submit"
              className="flex-1 py-2.5 rounded-xl bg-[#c9a96e] hover:bg-[#e8d5a3] text-[#1a1a1a] text-sm font-semibold transition-all cursor-pointer">Share Recipe</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <main className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          ...
          <div key={i} className="glass-card rounded-xl p-4 text-center border border-white/[0.04]">
            ...
            <div className="text-xl font-bold text-[#c9a96e]">{s.value}</div>
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        <input
          ...
          className="flex-1 min-w-[160px] px-3 py-2 rounded-xl bg-white/[0.04] border border-white/[0.08] text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-[#c9a96e]/50"
        />
        <div className="glass rounded-xl p-1 flex gap-1 border border-white/[0.04]">
          {(['All', 'Easy', 'Medium', 'Hard'] as const).map(d => (
            <button key={d} onClick={() => setFilter(d)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${
                filter === d ? 'bg-[#c9a96e]/20 text-[#e8d5a3] border border-[#c9a96e]/30' : 'text-gray-500 hover:text-gray-300'
              }`}
            >{d}</button>
          ))}
        </div>
