...
const DIFFICULTY_COLORS: Record<Difficulty, string> = {
  Easy: 'text-[#3A7D6F] bg-[#3A7D6F]/10 border-green-400/20',
  Medium: 'text-[#B09B71] bg-yellow-400/10 border-yellow-400/20',
  Hard: 'text-[#8B5A5A] bg-[#8B5A5A]/10 border-red-400/20',
};
...
function RecipeCard({ recipe, onClick }: { recipe: Recipe; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      className="glass-card rounded-2xl p-5 border border-white/[0.04] cursor-pointer hover:border-[#B09B71]/20 hover-lift transition-all"
    >
      <div className="text-3xl mb-3">{recipe.emoji}</div>
      <h3 className="text-base font-bold text-[rgba(245,240,232,0.90)] mb-1">{recipe.title}</h3>
      <p className="text-xs text-[rgba(245,240,232,0.35)] mb-3 line-clamp-2">{recipe.description}</p>
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[11px] text-[rgba(245,240,232,0.35)] bg-white/[0.04] px-2 py-0.5 rounded-full">{recipe.cuisine}</span>
        <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full border ${DIFFICULTY_COLORS[recipe.difficulty]}`}>
          {recipe.difficulty}
        </span>
        <span className="text-[11px] text-[rgba(245,240,232,0.35)] flex items-center gap-1">
          <Clock className="w-3 h-3" /> {formatPrepTime(recipe.prepTime)}
        </span>
      </div>
      <div className="mt-3 text-[10px] text-[rgba(245,240,232,0.25)]">Shared by {recipe.submittedBy}</div>
    </div>
  );
}
      ...
      <div className="glass w-full max-w-lg rounded-2xl border border-white/[0.08] max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-5 border-b border-white/[0.06]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-[rgba(245,240,232,0.90)]">{recipe.title}</h2>
              <div className="flex items-center gap-2 mt-0.5">
                <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full border ${DIFFICULTY_COLORS[recipe.difficulty]}`}>{recipe.difficulty}</span>
                <span className="text-[11px] text-[rgba(245,240,232,0.35)]">{recipe.cuisine}</span>
                <span className="text-[11px] text-[rgba(245,240,232,0.35)]">{formatPrepTime(recipe.prepTime)}</span>
              </div>
            </div>
          </div>
        ...
        <div className="p-5 space-y-5">
          <p className="text-sm text-[rgba(245,240,232,0.50)] italic">{recipe.description}</p>
          <div>
            <h3 className="text-xs font-bold text-[#B09B71] uppercase tracking-widest mb-3">Ingredients</h3>
            <ul className="space-y-1.5">
              {recipe.ingredients.map((ing, i) => (
                <li key={i} className="flex items-center gap-2 text-sm text-[rgba(245,240,232,0.65)]">
          ...
          <div>
            <h3 className="text-xs font-bold text-[#B09B71] uppercase tracking-widest mb-3">Instructions</h3>
            <ol className="space-y-3">
              {recipe.steps.map((step, i) => (
                <li key={i} className="flex items-start gap-3 text-sm text-[rgba(245,240,232,0.65)]">
                  <span className="w-6 h-6 rounded-full bg-[#B09B71]/20 text-[#B09B71] text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
                  {step}
                </li>
              ))}
            </ol>
          </div>
          <div className="text-[11px] text-[rgba(245,240,232,0.25)] pt-2 border-t border-white/[0.04]">
...
function AddRecipeModal({ onAdd, onClose }: { onAdd: (r: Recipe) => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
      <div className="glass w-full max-w-lg rounded-2xl border border-white/[0.08] max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-5 border-b border-white/[0.06]">
          <h2 className="text-base font-bold text-[rgba(245,240,232,0.90)]">Share a Recipe</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-[rgba(245,240,232,0.50)] cursor-pointer"><X className="w-4 h-4" /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-semibold text-[rgba(245,240,232,0.50)] mb-1 block">Title *</label>
            ...
            <div>
              <label className="text-xs font-semibold text-[rgba(245,240,232,0.50)] mb-1 block">Emoji</label>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="text-xs font-semibold text-[rgba(245,240,232,0.50)] mb-1 block">Cuisine</label>
              <input value={form.cuisine} onChange={e => setForm({...form, cuisine: e.target.value})} placeholder="Italian"
                className="w-full px-3 py-2 rounded-xl bg-white/[0.04] border border-white/[0.08] text-sm text-[rgba(245,240,232,0.80)] focus:outline-none focus:border-[#B09B71]/50" />
            </div>
            <div>
              <label className="text-xs font-semibold text-[rgba(245,240,232,0.50)] mb-1 block">Difficulty</label>
            ...
            <div>
              <label className="text-xs font-semibold text-[rgba(245,240,232,0.50)] mb-1 block">Prep (min)</label>
          ...
          <div>
            <label className="text-xs font-semibold text-[rgba(245,240,232,0.50)] mb-1 block">Description</label>
          ...
          <div>
            <label className="text-xs font-semibold text-[rgba(245,240,232,0.50)] mb-1 block">Ingredients (one per line)</label>
            <textarea rows={4} value={form.ingredients} onChange={e => setForm({...form, ingredients: e.target.value})} placeholder="2 cups flour&#10;1 tsp salt"
              className="w-full px-3 py-2 rounded-xl bg-white/[0.04] border border-white/[0.08] text-sm text-[rgba(245,240,232,0.80)] focus:outline-none focus:border-[#B09B71]/50 resize-none" />
          </div>
          <div>
            <label className="text-xs font-semibold text-[rgba(245,240,232,0.50)] mb-1 block">Steps (one per line)</label>
          ...
          <div className="flex gap-3">
            <button type="button" onClick={onClose}
              className="flex-1 py-2.5 rounded-xl border border-white/[0.08] text-sm text-[rgba(245,240,232,0.50)] hover:text-[rgba(245,240,232,0.80)] transition-all cursor-pointer">Cancel</button>
            <button type="submit"
              className="flex-1 py-2.5 rounded-xl bg-[#B09B71] hover:bg-[#D4C4A0] text-[#1a1a1a] text-sm font-semibold transition-all cursor-pointer">Share Recipe</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <main className="max-w-5xl mx-auto px-4 py-8">
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-[rgba(245,240,232,0.50)] text-sm mt-1">Recipes shared by your neighbors </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          ...
          <div key={i} className="glass-card rounded-xl p-4 text-center border border-white/[0.04]">
            ...
            <div className="text-xl font-bold text-[#B09B71]">{s.value}</div>
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        <input
          ...
          className="flex-1 min-w-[160px] px-3 py-2 rounded-xl bg-white/[0.04] border border-white/[0.08] text-sm text-[rgba(245,240,232,0.80)] placeholder-gray-600 focus:outline-none focus:border-[#B09B71]/50"
        />
        <div className="glass rounded-xl p-1 flex gap-1 border border-white/[0.04]">
          {(['All', 'Easy', 'Medium', 'Hard'] as const).map(d => (
            <button key={d} onClick={() => setFilter(d)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${
                filter === d ? 'bg-[#B09B71]/20 text-[#D4C4A0] border border-[#B09B71]/30' : 'text-[rgba(245,240,232,0.35)] hover:text-[rgba(245,240,232,0.65)]'
              }`}
            >{d}</button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {filtered.length === 0 && (
          <div className="col-span-3 text-center py-16 text-[rgba(245,240,232,0.35)]">
