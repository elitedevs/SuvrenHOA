...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button onClick={copy} className="p-1 rounded text-gray-600 hover:text-[#c9a96e] transition-colors">
...
function AddressDisplay({ address, basescanUrl }: { address: string; basescanUrl: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <code className="text-xs font-mono text-[#c9a96e] bg-[#c9a96e]/10 px-2 py-1 rounded">{short}</code>
      <CopyButton text={address} />
      <a href={basescanUrl} target="_blank" rel="noopener noreferrer" className="p-1 rounded text-gray-600 hover:text-[#c9a96e] transition-colors">
...
function ContractCard({
  ...
  return (
    <div className="glass-card rounded-2xl p-6 border border-[#c9a96e]/10 hover:border-[#c9a96e]/25 transition-all duration-200">
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#c9a96e]/10 border border-[#c9a96e]/20 flex items-center justify-center text-xl">
            {emoji}
          </div>
          <div>
            <h3 className="font-bold text-[#e8d5a3] text-sm">{name}</h3>
            <p className="text-xs text-gray-500 mt-0.5">EVM Smart Contract</p>
          </div>
        </div>
        <a href={basescanUrl} target="_blank" rel="noopener noreferrer"
          className="flex items-center gap-1 text-xs text-[#c9a96e] hover:text-[#e8d5a3] transition-colors">
      ...
      {/* Address */}
      <div className="mb-4">
        <p className="text-[10px] text-gray-600 uppercase tracking-wide mb-1.5 font-semibold">Contract Address</p>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        {stats.map(s => (
          <div key={s.label} className="bg-white/3 rounded-xl p-3">
            <p className="text-[10px] text-gray-600 uppercase tracking-wide font-semibold mb-1">{s.label}</p>
      ...
      {showFunctions && (
        <div className="mt-2 space-y-1">
          {functions.map(fn => (
            <code key={fn} className="block text-[11px] text-gray-400 font-mono bg-white/3 px-2 py-1 rounded">
              {fn}
            </code>
          ))}
        </div>
...
export default function ContractsPage() {
  ...
  if (!isConnected) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        <p className="text-gray-400 mb-4">Sign in to view smart contracts</p>
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-10 page-enter">
      {/* Header */}
      <div className="mb-10">
        <p className="text-xs text-gray-500 font-semibold uppercase tracking-widest mb-1">Blockchain</p>
        <h1 className="text-3xl font-extrabold tracking-tight">Smart Contract Explorer</h1>
        <p className="text-gray-400 text-sm mt-2">All SuvrenHOA contracts deployed on Base {isMainnet ? 'Mainnet' : 'Sepolia (testnet)'}</p>
        <div className="inline-flex items-center gap-2 mt-3 px-3 py-1.5 rounded-full bg-[#c9a96e]/10 border border-[#c9a96e]/20 text-xs text-[#c9a96e]">
