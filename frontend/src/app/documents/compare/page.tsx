  ...
  return (
    <div className="min-h-screen bg-[oklch(0.06_0.005_60)] p-6 lg:p-10">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <Link href="/documents" className="flex items-center gap-1.5 text-xs text-[oklch(0.45_0.01_60)] hover:text-[#c9a96e] transition-colors mb-4">
          ...
          <h1 className="text-3xl font-bold text-[#e8d5a3] mb-2 flex items-center gap-3">
            <GitCompare className="w-8 h-8 text-[#c9a96e]" /> Document Comparison
          </h1>
          ...
          {[
            ...
            <div key={label}>
              ...
              <select
                ...
                className={`w-full bg-[oklch(0.12_0.005_60)] border-2 ${color} rounded-xl px-4 py-3 text-sm text-[#e8d5a3] focus:outline-none focus:border-[#c9a96e] transition-colors`}
              >
                {SAMPLE_DOCS.map(d => (
        ...
        <div className="flex items-center justify-between flex-wrap gap-4 mb-5">
          <div className="flex gap-3 text-sm">
            <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-green-950/30 border border-green-700/30 text-green-400">
              <Plus className="w-3.5 h-3.5" /> {added} added
            </span>
            <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-950/30 border border-red-700/30 text-red-400">
          ...
          <div className="flex gap-1.5 p-1 bg-[oklch(0.10_0.005_60)] rounded-xl border border-[oklch(0.18_0.005_60)]">
            {(['unified', 'split'] as const).map(v => (
              <button key={v} onClick={() => setView(v)} className={`px-4 py-1.5 rounded-lg text-xs font-medium capitalize transition-colors ${view === v ? 'bg-[#b8942e] text-[#1a1a1a]' : 'text-[oklch(0.50_0.01_60)] hover:text-[#e8d5a3]'}`}>{v}</button>
            ))}
          </div>
        </div>
          ...
          view === 'unified' ? (
            <div className="bg-[oklch(0.10_0.005_60)] border border-[oklch(0.18_0.005_60)] rounded-2xl overflow-hidden">
              <div className="flex items-center gap-4 px-4 py-3 bg-[oklch(0.12_0.005_60)] border-b border-[oklch(0.18_0.005_60)] text-xs text-[oklch(0.45_0.01_60)]">
                <span className="text-red-400 font-mono">--- {a.title} ({a.version})</span>
                <span className="text-green-400 font-mono">+++ {b.title} ({b.version})</span>
              </div>
              <div className="overflow-x-auto">
                <div className="font-mono text-xs">
                  {diff.map((line, i) => (
                    <div key={i} className={`flex items-start px-4 py-0.5 ${
                      line.type === 'added' ? 'bg-green-950/30 border-l-2 border-green-600'
                      : line.type === 'removed' ? 'bg-red-950/30 border-l-2 border-red-600'
                      : ''
                    }`}>
                      <span className={`w-5 shrink-0 text-center mr-3 ${
                        line.type === 'added' ? 'text-green-400' : line.type === 'removed' ? 'text-red-400' : 'text-[oklch(0.30_0.005_60)]'
                      }`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={label} className={`bg-[oklch(0.10_0.005_60)] border-2 ${color} rounded-2xl overflow-hidden`}>
                  <div className={`px-4 py-3 border-b ${color} bg-[oklch(0.12_0.005_60)] text-xs font-medium ${label === 'Original' ? 'text-red-400' : 'text-green-400'}`}>
                    {label}: {doc.title} ({doc.version})
                  </div>
                  <div className="overflow-x-auto p-4 font-mono text-xs space-y-0.5">
                    {lines.map((line, i) => (
                      <div key={i} className={`px-2 py-0.5 rounded ${line.type === 'removed' ? 'bg-red-950/30 text-red-300 line-through' : line.type === 'added' ? 'bg-green-950/30 text-green-200' : 'text-[oklch(0.60_0.01_60)]'}`}>
                        {line.line || '\u00A0'}
                      </div>
                    ))}
                  </div>
